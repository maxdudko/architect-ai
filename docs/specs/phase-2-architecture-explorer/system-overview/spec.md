...
- **Q-1.** Is the overview represented as a new member of the
  existing generated-document type set, or as its own entity?
  Verification established that adding a declared type without a
  registered generator would break every post-index generation
  run, because requested types default to all declared types and
  the registry throws for unregistered types (§0.7).
- **Q-2.** If the overview reuses the existing document type
  set, is it included in automatic post-index generation, and
  how is the default request set controlled so existing
  behavior does not change silently (FR-18)?
- **Q-3.** What is the concurrency policy for overlapping
  requests (FR-12), given that the existing pipeline returns any
  active run for the repository regardless of requested types?
- **Q-4.** How is the generation allowance consumed (FR-16),
  given that it is counted from generation run rows, and how is
  an unmetered path ruled out?
- **Q-5.** What exact document structure and required section
  set does the overview use (FR-7), and how does it relate to
  the existing contract-driven heading model?
- **Q-6.** Which single source reference shape does Phase 2
  adopt (EV-2), and is structural evidence recorded as
  structured data or only as cited paths in metadata, as
  existing guides do?
- **Q-7.** What are the concrete bounds and context budget
  required by LG-4 and LG-5?
- **Q-8.** What happens when the architecture model is
  unavailable or empty for a revision (AD-1, ST-5) — refuse, or
  produce a document with a stated reduced basis?
- **Q-9.** How is generated content validated against the
  evidence set (AD-8, EV-3), and what constitutes a validation
  failure severe enough to fail the run?
- **Q-10.** How is the target revision selected and carried
  through generation (AD-5, IR-4) so that no stage independently
  re-resolves "latest successful revision"?
- **Q-11.** What is the product relationship between the
  overview and the existing executive summary and project
  overview guides (FR-19) — replace, supersede, coexist with
  distinct purposes, or re-base the existing ones on the
  architecture model?
- **Q-12.** Is regeneration ever triggered automatically, for
  example after a successful reindex, or only on request?
- **Q-13.** Does the overview include any diagrammatic
  representation, or is it prose and structured lists only?
- **Q-14.** Is the overview retained when the repository is
  reindexed, disconnected, or soft-deleted, and for how long?
- **Q-15.** Which automated tests cover each acceptance
  criterion, and what fixture repository provides known modules
  and known resolvable dependencies for AC-4, plus a known
  insufficient-data repository for AC-10?
